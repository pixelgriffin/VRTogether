...
namespace VRArmIK
{
	public class AvatarVRTrackingReferences : MonoBehaviour
	{
		...
		{
			createTransform(ref head, head.name);
			createTransform(ref leftHand, leftHand.name);
			createTransform(ref rightHand, rightHand.name);
			createTransform(ref hmd, hmd.name);
		}
